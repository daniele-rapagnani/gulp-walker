		...
		for (var j = 0; j < dependencies.length; j++) {
			...
			if (this.resolvers.hasOwnProperty(extension)) {
				for (var k = 0; k < this.resolvers[extension].length; k++) {
					var absPaths = this.resolvers[extension][k].resolve(filePath, dependencies[j]);

					if (absPaths.length) {
						dependencyPaths = dependencyPaths.concat(absPaths);
					} else {
						Logger.debug("Can't resolve file:", dependencies[j]);
					}
				}
			} else {
				dependencyPaths.push(dependencies[j]);
			}

			depFiles = depFiles.concat(dependencyPaths);
		}
	}
