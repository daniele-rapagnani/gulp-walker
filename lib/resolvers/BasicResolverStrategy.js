...
BasicResolverStrategy.prototype.resolve = function(includerPath, filePath) {
	...
	if (pathAbsolute || pathNotExplicit) {
		...
		paths = paths.concat(pathGuesses);
	}

	guesses = guesses.concat(this.guessFileExtension(paths));

	Logger.debug(filePath, "resolved to this possible paths", guesses);

	var foundFiles = this.getFilesForFirstMatch(guesses);
	var includedFiles = [];

	if (foundFiles.length) {
		for (var i = 0; i < foundFiles.length; i++) {
			var statInfo = fs.lstatSync(foundFiles[i]);

			if (statInfo.isDirectory()) {
				Logger.debug(foundFiles[i], "is a directory, trying to guess index file");
				includedFiles = includedFiles.concat(this.resolve(
					includerPath, this.guessDirectoryIndex(foundFiles[i], filePath)
				));
			} else {
				includedFiles.push(foundFiles[i]);
			}
		};

		Logger.info(filePath, "was resolved to", includedFiles);
	} else {
 ...
 * and returns the first that resolves to one
 * or more files (if it is a glob pattern).
 *
 * @method
 * @param {Array} absPaths An array of absolute paths to
 *		files.
 *
 * @return {Array} An array of the absolute paths of files or
 *					an empty array.
 */

BasicResolverStrategy.prototype.getFilesForFirstMatch = function(absPaths) {
	for (var i = 0; i < absPaths.length; i++) {
		if (this.config.allowGlobbing) {
			var files = glob.sync(absPaths[i]);
